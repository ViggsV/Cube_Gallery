...
scenes.forEach(function (scene) {
  ...
  style.innerHTML = `
    @keyframes ${animationName} {
      ...
      50% {
        transform: translateZ(-512px) translateY(0px) rotateX(${
          randomX + directionX * 180
        }deg) rotateY(${randomY + directionY * 180}deg) rotateZ(${
    randomZ + directionZ * 180
  }deg);
      }
      100% {
        transform: translateZ(${randomZZ}px) translateY(${randomYY}px) rotateX(${
    randomX + directionX * 360
  }deg) rotateY(${randomY + directionY * 360}deg) rotateZ(${
    randomZ + directionZ * 360
  }deg);
      }
    }
  `;
    ...
    if (!isTransitioning) {
      if (cube.classList.contains("paused")) {
        ...
        labels.forEach(function (label) {
          // Loop over each label and update its opacity
      ...
      `;
        ...
        setTimeout(() => {
          cube.style.transform =
            "translateZ(-512px) rotateX(0deg) rotateY(0deg) rotateZ(0deg)";
